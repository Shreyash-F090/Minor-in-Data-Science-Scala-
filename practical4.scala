...
object practical4 {
    def main(args: Array[String]): Unit = {
      ...
      println(f"Sum of Dense Vector S1 is : $sum%.2f")

      val mean = breeze.stats.mean(S2)
      println(f"Mean of Dense Vector S2 is : $mean%.2f")

      val Dotproduct = S1 dot S2
      println(f"Dot product of S1 and S2 Dense Vector is : $Dotproduct%.3f")
